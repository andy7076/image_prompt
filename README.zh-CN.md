...
> 一个收录 570+ 图片 Prompt、支持编辑与浏览器内生成的视觉案例库。
...
PROMPT/SIGNAL 将公开社区中的高质量图片 Prompt 整理成实用工作台。发现案例、编辑 Prompt 与变量，可以继续交给 ChatGPT 或 Gemini 使用，也可以通过当前浏览器中配置的图片模型直接生成。

## 核心能力

| 能力 | 说明 |
| --- | --- |
| 精准瀑布流 | 570+ 案例以最短列算法均衡排列，支持搜索、分类、排序、收藏和稳定加载占位。 |
| Prompt 工作台 | 编辑完整 Prompt 与模板变量、恢复原文、查看多出处来源、放大图片，并切换多图案例。 |
| 外部平台接力 | 短 Prompt 可直接带入 ChatGPT；长 Prompt 与 Gemini 流程会先复制当前编辑内容，再打开新标签页。 |
| 生成工作台 | 从案例或空白 Prompt 开始，添加最多 8 张参考图，选择模型与本次参数，确认后再发送。 |
| 多模型 Profile | 在本地保存多套 Images API 或 GenerateContent API 配置，每次生成时可以自由切换。 |
| 本地记录 | 收藏、语言与主题偏好、模型配置，以及最多 30 条生成结果均保存在当前浏览器。 |
...
点击右上角的 **模型**，添加或选择一个 Profile 后保存。配置表单不绑定任何特定服务商，只需填写接口文档中对应的协议、地址、密钥和模型标识。首次访问时 URL、Key、Model 均为空，`SIZE` 和 `QUALITY` 默认 `auto`。
...
| `QUALITY` | Images API 可选 `auto`、`low`、`medium`、`high`、`standard` 或 `hd`。 |

Profile 中的 `SIZE` 和 `QUALITY` 是默认值。每次生成时都可以临时修改，不会覆盖已保存的 Profile。
...
上传参考图后，应用会先在浏览器中对每张图片进行无损优化，再将末尾的 `/generations` 自动替换为 `/edits`，并以 `multipart/form-data` 发送。单张图片字段为 `image`，多张图片字段为 `image[]`。接口需要在响应中返回 `data[0].url` 或 `data[0].b64_json`。
...
### 在 ChatGPT 或 Gemini 中继续

1. 打开案例，编辑 Prompt 或模板变量。
2. 点击 **使用模型生成** 右侧的 ChatGPT 或 Gemini 图标。
3. 阅读接力说明并确认。
4. 较短的 ChatGPT Prompt 会直接加入跳转链接；长 Prompt 和 Gemini 流程会先复制当前内容，再打开新标签页供手动粘贴。

使用剪贴板时，浏览器可能请求复制权限。如果复制失败，页面会明确提示，不会让用户误用剪贴板里残留的旧内容。

### 使用已配置的模型生成

1. 打开案例，或点击 **自由创作** 从完全自定义的 Prompt 开始。
2. 编辑 Prompt，并按需上传最多 8 张 PNG、JPEG 或 WEBP 参考图。
3. 选择已保存的模型 Profile，并设置本次请求的 `SIZE` 与 `QUALITY`。
4. 点击 **使用模型生成**，检查确认窗口后开始生成。
5. 在 **生成记录** 中重新打开结果，复制 Prompt、下载图片或继续生成新版本。

参考图会先在本地完成无损优化。远程图片不支持直接下载时，生成结果会在新标签页打开，不会替换当前页面。

## 本地数据与隐私

- 模型 Profile、API Key、收藏、偏好和生成记录只保存在当前浏览器。
- Prompt 与参考图由浏览器直接发送到当前选择的接口，本仓库不提供中转服务器。
- 跳转 ChatGPT/Gemini 前始终会展示确认窗口。
- 清除浏览器存储会同时删除本地配置和历史记录，请勿在共享设备中保存生产环境密钥。

顶部可以切换英文/中文和亮色/暗色主题，默认使用亮色主题。
